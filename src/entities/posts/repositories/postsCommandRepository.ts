...
class PostsCommandRepository {
  static async findById(postId: string) {
    ...
    return PostsCommandRepository._cleanObjectIdMapper(foundPost);
  }

  static async checkById(postId: string) {
  ...
  static async save(inputPost: PostType): Promise<string> {
    const { insertedId } = await postsCollection.insertOne(inputPost);
    return insertedId.toString();
  }

  static async update(postId: string, inputPost: InputUpdatePostType) {
  ...
  static async removeRelated(blogId: string) {
    const deleteResult = await postsCollection.deleteMany({ blogId });
    return deleteResult.deletedCount !== 0;
  }

  static async updateRelated(blogId: string, blogName: string) {
  ...
  static _cleanObjectIdMapper(mongoUser: WithId<PostType>): ViewPostType {
    return {
...
const postsCommandRepository = PostsCommandRepository;
